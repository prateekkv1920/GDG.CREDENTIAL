...
const btnAutofill1 = document.getElementById('btn-autofill-1');
const btnAutofill2 = document.getElementById('btn-autofill-2');

if (btnAutofill1) {
  btnAutofill1.addEventListener('click', () => {
    emailInput.value = 'chanchalkadem@gmail.com';
    passwordInput.value = '#devhack@1';
    loginForm.requestSubmit();
  });
}

if (btnAutofill2) {
  btnAutofill2.addEventListener('click', () => {
    emailInput.value = 'jyotiriya26@gmail.com';
    passwordInput.value = '#devhack@2';
    loginForm.requestSubmit();
  });
}

// ── Auto-Seed Test Participants if Local Database is Empty ──
async function ensureSampleDataSeeded() {
  try {
    const existing1 = await getParticipantByEmailLocal('chanchalkadem@gmail.com');
    if (!existing1) {
      ...
      if (img1Resp.ok && img2Resp.ok) {
        const [blob1, blob2] = await Promise.all([img1Resp.blob(), img2Resp.blob()]);
        const reader1 = new FileReader();
        const reader2 = new FileReader();

        const p1 = new Promise(res => { reader1.onload = () => res(reader1.result); reader1.readAsDataURL(blob1); });
        const p2 = new Promise(res => { reader2.onload = () => res(reader2.result); reader2.readAsDataURL(blob2); });

        const [url1, url2] = await Promise.all([p1, p2]);
        const hash1 = await hashPassword('#devhack@1');
        const hash2 = await hashPassword('#devhack@2');

        const { saveParticipantLocal } = await import('./storage-adapter.js');
        await saveParticipantLocal({
          name: 'KM chanchal',
          email: 'chanchalkadem@gmail.com',
          passwordHash: hash1,
          certificateUrl: url1,
          certificateFilename: '1.png',
          eventName: 'DevHack 2026',
          createdAt: new Date().toISOString()
        });

        await saveParticipantLocal({
          name: 'Jyoti Hansdah',
          email: 'jyotiriya26@gmail.com',
          passwordHash: hash2,
          certificateUrl: url2,
          certificateFilename: '2.png',
          eventName: 'DevHack 2026',
          createdAt: new Date().toISOString()
        });
      }
